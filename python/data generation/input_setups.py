...
def input_ftsuoe(n, posdata, frameno, timestep, last_velocities, input_form='undefined', video=False,
    ...
    (sphere_sizes, sphere_positions, sphere_rotations, dumbbell_sizes, dumbbell_positions, dumbbell_deltax, num_spheres,
     num_dumbbells, element_sizes, element_positions, element_deltax, num_elements, num_elements_array, element_type,
     uv_start, uv_size, element_start_count) = posdata_data(posdata)
    ...
    if n == 1:
        ...
        Fa_in[:] = [[0, 0, -1] for i in xrange(num_spheres)]
        sphere_positions, box_bottom_left, box_top_right = simple_cubic_8(32)
        ...
        DFb_in[:] = [list(spring_constant * (
                dumbbell_deltax[i] - natural_deltax * dumbbell_deltax[i] / np.linalg.norm(dumbbell_deltax[i]))) for
                     i in range(num_dumbbells)]
        # Simple shear with speed gammadot
        startfromframe = 0
        # amplitude is amplitude at z = 1
        (Ea_in, U_infinity, O_infinity, centre_of_background_flow, amplitude, frequency) = oscillatory_shear(
            amplitude=1. / 3., period=1, start_from_frame=0, centre_of_background_flow=np.array([2.25, 0, 2.25]),
            frameno=frameno, timestep=timestep, num_spheres=num_spheres)
        desc = "oscillatory-background-flow"

    elif n == 4:
        # Repulsive force
        (Fa_in, Fb_in, DFb_in) = repulsion_forces(100, 20, num_spheres, num_dumbbells, sphere_positions,
                                                  dumbbell_positions, dumbbell_deltax, sphere_sizes, dumbbell_sizes,
                                                  num_sphere_in_each_lid, Fa_in, Fb_in, DFb_in)
        desc = "repulsion"

    elif n == 5:
        # Force half the spheres to move to the left with a given velocity, and force the rest to move to the right.
        Ua_in[:] = [[-1, 0, 0] for i in xrange(num_spheres / 2)] + [[1, 0, 0] for i in
                                                                    xrange(num_spheres / 2, num_spheres)]
        ...
        # torque
        #        Ta_in[:] = [[0,1,0] for i in xrange(num_spheres)]
        Ta_in[0] = [0, 0, 0]
        Ta_in[1] = [0, 1, 0]
        # Ta_in[2] = [0, 0, 0]
        sphere_positions, box_bottom_left, box_top_right = simple_cubic_8(40)
        desc = "torque"
...
def repulsion_forces(strength, tau, num_spheres, num_dumbbells, sphere_positions, dumbbell_positions, dumbbell_deltax,
                     sphere_sizes, dumbbell_sizes, num_sphere_in_each_lid, Fa_in, Fb_in, DFb_in,
                     last_velocities=[0, 0, 0]):
    bead_force = [[0, 0, 0] for i in xrange(num_spheres + 2 * num_dumbbells)]
    bead_positions = np.concatenate(
        [sphere_positions, dumbbell_positions - 0.5 * dumbbell_deltax, dumbbell_positions + 0.5 * dumbbell_deltax])
    ...
    overlapping_or_close = np.where(
        np.logical_and(scaled_distance_matrix > 0, np.logical_and(scaled_distance_matrix < cutoff, index_matrix >= 0)))
    ...
    for pair in overlapping_or_close_pairs:
        ...
        h = scaled_overlap - 2.
        unit_vector = (bead_positions[pair[1]] - bead_positions[pair[0]]) / np.linalg.norm(
            (bead_positions[pair[1]] - bead_positions[pair[0]]))
    ...
    Fa_in[2 * num_sphere_in_each_lid:num_spheres] = Fa_in[2 * num_sphere_in_each_lid:num_spheres] - np.array(
        bead_force[2 * num_sphere_in_each_lid:num_spheres])
    Fb_in = Fb_in - (np.array(bead_force[num_spheres:num_spheres + num_dumbbells]) + np.array(
        bead_force[num_spheres + num_dumbbells:]))
    DFb_in = DFb_in - (np.array(bead_force[num_spheres + num_dumbbells:]) - np.array(
        bead_force[num_spheres:num_spheres + num_dumbbells]))
    return Fa_in, Fb_in, DFb_in


def oscillatory_shear(amplitude, period, start_from_frame, frameno, timestep,
                      centre_of_background_flow=np.array([0, 0, 0]), num_spheres=1, unused_axis=1,
                      transpose_shear=False, opposite_direction=False):
    ...
    if unused_axis == 1:
        ...
        Ea_in = [[[0, opposite_minus * 0.5 * gammadot, 0], [opposite_minus * 0.5 * gammadot, 0, 0], [0, 0, 0]] for i in
                 xrange(max(1, num_spheres))]
