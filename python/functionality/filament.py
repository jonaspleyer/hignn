...
def velocity_update(t, position):
    ...
    k_t = 20 # tension stiffness coefficient
    k_b = 40 # bending stiffness coefficient
    rest_length = 2.4
...
if __name__ == '__main__':    
    ...
    hignn_model = hignn.HignnModel(X, 10) # the minimal number of particles in a leaf node
    ...
    for i in range(num_ite):
        if i % num_output == 0:
            with h5py.File('Result/pos'+str(int(i/num_output))+'rank'+str(rank)+'.h5', 'w') as f:
        ...
        if i % num_output == 0:
            with h5py.File('Result/vel'+str(int(i/num_output))+'rank'+str(rank)+'.h5', 'w') as f:
