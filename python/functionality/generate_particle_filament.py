...
if __name__ == '__main__':
    ...
    R1 = (1/2)**(1/3) * R2
    n_particle = n_fiber0 * n_chain - n_fiber * n_chain
    
    X = np.zeros((n_chain * n_fiber0, 3), dtype=np.float32)
    ...
    while n < n_fiber:
        print('N =', n, end='\r')
        num_generated_fiber = min(10240, n_fiber - n)
        # generate candidate center points
        X_candidate = 2*np.random.rand(num_generated_fiber*3)*R1-R1
        X_candidate = X_candidate.reshape((num_generated_fiber, 3))
        r = np.linalg.norm(X_candidate, axis=1)
        X_candidate = np.reshape(X_candidate[(r < R1).nonzero(), :], (-1, 3))
    ...
    while n < n_particle:
        ...
        X[n_chain * n_fiber + n:n_chain * n_fiber + n + selected_num, :] = X_candidate[:selected_num, :]
        n += selected_num
    
    np.savetxt('initial/particle_filament_l'+str(n_chain)+'_'+str(n_fiber)+'.txt', X)