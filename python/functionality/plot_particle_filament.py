...
for N in range(500):
    ...
    for rank in range(4):
        try:
            points_array_rank = h5py.File('Result/particle_filament_l_'+str(n_chain)+'_kt_'+str(k_t)+'_kb_'+str(k_b)+'/pos_rank_'+str(rank)+'_N_'+str(num_filaments)+"_"+str(N)+'.h5', 'r')['pos'][:]
            if points_array is None:
                points_array = points_array_rank
            else:
                points_array = np.concatenate((points_array, points_array_rank), axis=0)
            velocity_array_rank = h5py.File('Result/particle_filament_l_'+str(n_chain)+'_kt_'+str(k_t)+'_kb_'+str(k_b)+'/vel_rank_'+str(rank)+'_N_'+str(num_filaments)+"_"+str(N)+'.h5', 'r')['vel'][:]
    ...
    particle_start = num_filaments * n_chain
    ...
    # writer.SetFileName('visualization/particle_filament_l_'+str(n_chain)+'_kt_'+str(k_t)+'_kb_'+str(k_b)+'/original_N_'+str(num_filaments)+"_"+str(N)+'.vtp')
    writer.SetFileName('visualization/particle_filament_l_'+str(n_chain)+'_kt_'+str(k_t)+'_kb_'+str(k_b)+'/original_filament_only_N_'+str(num_filaments)+"_"+str(N)+'.vtp')
    writer.Write()