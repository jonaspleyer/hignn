...
for N in range(500):
    # if os.path.exists('Result/output_trans'+str(N)+'.vtp'):
    #     continue
    ...
    for rank in range(1):
        try:
            # points_array = np.loadtxt('Result/pos'+str(N)+'rank'+str(rank)+'.txt', dtype=np.float32)
            # velocity_array = np.loadtxt('Result/vel'+str(N)+'rank'+str(rank)+'.txt', dtype=np.float32)
    ...
    writer.SetFileName('Result/w_multi_filament'+str(N)+'.vtp')
    writer.Write()